...
def brun(sess, y, a):
    ...
    for i in range(number_of_full_batch):
        preds += list(sess.run(y,
                               feed_dict={input_x: a[i*batch_size:(i+1)*batch_size], kr: 1.0}))
    return preds
...
with open(inp) as f:
    for line in f:
        if(line.startswith(">")):
            ...
            if(len(seq) != 0):
                sequences1.append(np.fromstring(
                    encode(seq), dtype=int, sep=","))
                seq = ""
            continue
        else:
            seq += line
...
with tf.Session(graph=new_graph) as sess:
    ...
    tf.saved_model.loader.load(
        sess, [tf.saved_model.tag_constants.SERVING], sys.argv[1])
