...
def conv1d(input, name, kshape, strides=[1, 1, 1, 1]):
    with tf.name_scope(name):
        W = tf.get_variable(name='fre_w_'+name,
                            shape=kshape)
        b = tf.get_variable(name='fre_bias_' + name,
                            shape=[kshape[2]])
        out = tf.nn.conv1d(input,W,stride=1, padding='SAME')
...
# define the graph
def model_graph(x, keep_ratio):
    f1 = conv1d(x, name='f1', kshape=[15, 4, 30])
    f2 = conv1d(f1, name='f2', kshape=[15, 30, 60])

    #s1 = conv1d(x, name='s1', kshape=[15, 4, 60])
    #s2 = tf.layers.max_pooling1d(s1, 15, 1, padding='same')
   
    #fc = tf.contrib.layers.flatten(tf.concat([f2, s2], 2))
    #print("-----------------------------------------------------------------------")
    #print(fc)
...
'testing_split':0.3,'validation_split':0.1,'additional_negatives':0.0,
 'patience':10,  'read_model': 0, 'test_results_file':'results', 'threshold':0, 'kr':1.0, 'cw1':1.0, 'model_file': 'model.h5',
...
sl = len(pos_seq[0])
x_data = zeros((len(pos_seq) + len(neg_seq),  sl, 4))
...
tf.identity(kr, name="kr")
y_logit, y = model_graph(input_x, kr)
tf.identity(y, name="output_prom")
tf.identity(kr, name="kr_prom")
...
for epoch in range(nb_epoch):
    ...
    #    pred += list(sess.run(y, feed_dict={input_x: x_train[i*batch_size:(i+1)*batch_size], kr: 1.0}))
    ...
    for i in range(number_of_full_batch):
        pred += list(sess.run(y, feed_dict={input_x: x_val[i*batch_size:(i+1)*batch_size], kr: 1.0}))
    pred = np.asarray(pred)
