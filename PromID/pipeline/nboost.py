...
def brun(sess, y, a):
    ...
    for i in range(number_of_full_batch):
        preds += list(sess.run(y, feed_dict={input_x: a[i*batch_size:(i+1)*batch_size], kr: 1.0}))
    return preds
...
sequences = zeros((len(sequences1),  len(sequences1[0]), 4))
...
with tf.Session(graph=new_graph) as sess:
    ...
    kr = tf.get_default_graph().get_tensor_by_name("Placeholder_1:0")
    for i in range(len(sequences)):
